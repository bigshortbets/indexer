...
import {LiquidationPrice, Market, OrderSide, Position} from "../../model";

export class LiquidationPriceCalculator {
    public static async calculate(position: Position, store: Store, quantityDelta: bigint) {
        ...
        shortsideLiquidationPrice = LiquidationPriceCalculator.processPositionSide(shortsideLiquidationPrice, quantityDelta, position, OrderSide.SHORT);
        longsideLiquidationPrice = LiquidationPriceCalculator.processPositionSide(longsideLiquidationPrice, quantityDelta, position, OrderSide.LONG);

        return await store.save([longsideLiquidationPrice, shortsideLiquidationPrice])
    }
    private static processPositionSide(liquidationPrice: LiquidationPrice | undefined, quantityDelta: bigint, position: Position, side: OrderSide) : LiquidationPrice {
        if(!liquidationPrice) {
            liquidationPrice = new LiquidationPrice({id: position.id + (side === OrderSide.LONG ? "01" : "00"),
