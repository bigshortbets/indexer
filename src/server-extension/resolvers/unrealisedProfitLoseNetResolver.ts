...
export class UnrealisedProfitLoseNetResolver {
  ...
  async calculateUnrealisedPLNetPerUser(
    @Arg("who", { nullable: false }) who: string,
    ...
    if (market) {
      ...
      return new UnrealisedProfitLoseNetResult({
        value: (shortSum + longSum).toString(),
      });
    } else {
